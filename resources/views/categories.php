...
if ($stmt->rowCount() > 0) {
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<td>" . $i . "</td>";
        echo "<td>" . $row['name'] . "</td>";
        echo "<td><a class=\"btn btn-info btn-sm\" href=\"books.php?category_id={$row['id']}\">{$bookcount['count']} Books</a>";
        echo " <a class=\"btn btn-success btn-sm\" href=\"categoryEdit.php?id={$row['id']}\">Edit</a>";

        echo " <a class=\"btn btn-danger btn-sm\" href=\"../../src/category.php?id={$row['id']}\" onClick=\"if(!confirm('Are you sure'))return false;\">Delete</a>";
